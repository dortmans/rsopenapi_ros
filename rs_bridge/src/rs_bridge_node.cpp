...
public:
  RsBridgeNode() : Node("rs_bridge")
  {
    ...
    // Create ROS timer, publishers, subscribers, actions, broadcasters
    ...
    obstacles_publisher_ = this->create_publisher<geometry_msgs::msg::PoseArray>("obstacles", 10);
    ...
    geometry_msgs::msg::TransformStamped tf_map_ros_to_msl;
    tf_map_ros_to_msl.header.stamp = this->get_clock()->now();
    tf_map_ros_to_msl.header.frame_id = "map";
    tf_map_ros_to_msl.child_frame_id = "map_msl";
    // q.setRPY(0.0, 0.0, -M_PI_2);
    // tf_map_ros_to_msl.transform.rotation = tf2::toMsg(q);
    tf_static_broadcaster_->sendTransform(tf_map_ros_to_msl);

    geometry_msgs::msg::TransformStamped tf_base_link_ros_to_msl;
    tf_base_link_ros_to_msl.header.stamp = this->get_clock()->now();
    tf_base_link_ros_to_msl.header.frame_id = "base_link";
    tf_base_link_ros_to_msl.child_frame_id = "base_link_msl";
    q.setRPY(0.0, 0.0, -M_PI_2);
    tf_base_link_ros_to_msl.transform.rotation = tf2::toMsg(q);
    tf_static_broadcaster_->sendTransform(tf_base_link_ros_to_msl);

    geometry_msgs::msg::TransformStamped tf_map_to_odom;
    tf_map_to_odom.header.stamp = this->get_clock()->now();
    tf_map_to_odom.header.frame_id = "map";
    tf_map_to_odom.child_frame_id = "odom";
    tf_static_broadcaster_->sendTransform(tf_map_to_odom);
...
private:
  ...
  template <typename T>
  bool tf_transform(
      const T &input_stamped,
      T &output_stamped,
      const std::string &target_frame)
  {
    try
    {
      output_stamped = tf_buffer_->transform(
          input_stamped, target_frame,
          tf2::durationFromSec(1.0));
      output_stamped.header.frame_id = target_frame;
      return true;
    }
  ...
  geometry_msgs::msg::PoseStamped to_ros_pose(
      ...
      double rz)
  /*
   * Position (x, y, z) and angle (rz) are relative to world coordinate system (map_msl)
   */
  {
    ...
    double yaw = rz + M_PI_2; // MSL: rz is angle from x-axis. Yaw is angle from y-axis
    tf2::Quaternion q;
   ...
   */
  {
    ...
    // Transform velocity vector to ROS robot coordinate system (map)
  ...
  void timer_callback()
  {
    ...
    if (robot_->read(data_))
    {
      ...
      obstacles.header.stamp = ros_stamp_from_ts(data_.metadata.ts);
      for (auto &obstacle : data_.obstacles)
      {
        obstacles.header.stamp = ros_stamp_from_ts(obstacle.ts);
        ros_pose = to_ros_pose(obstacle.pose.x, obstacle.pose.y, 0, obstacle.pose.rz);
        obstacles.poses.push_back(ros_pose.pose);
      }
      obstacles_publisher_->publish(obstacles);
  ...
  rclcpp::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr obstacles_publisher_;
